...
(deftest handle-close-paren-test-matches
  (is (let [close-paren ")"
            stack (atom ["("])
            fail-atom (atom false)]
        (matching-brackets/handle-close-paren stack close-paren fail-atom)
        (and
         (true? (empty? @stack))
         (false? @fail-atom)))))

(deftest handle-close-paren-test-matches-squares
  (is (let [close-paren "]"
            stack (atom ["["])
            fail-atom (atom false)]
        (matching-brackets/handle-close-paren stack close-paren fail-atom)
...
(deftest handle-close-paren-test-fails
  (is (let [close-paren "]"
            stack (atom ["("])
            fail-atom (atom false)]
        (matching-brackets/handle-close-paren stack close-paren fail-atom)
